...
def append_to_csv_remove_first(file_name, row):
    file_exists = os.path.isfile(file_name)
    lines = []
    ...
    if file_exists:
        ...
        # Remove the first data line if there are more than just the header
        if len(lines) > 1:
            lines.pop(1)
    ...
    with open(file_name, 'w', newline='') as csvfile:
        ...
        if not file_exists:
            ...
            writer.writerow(['Date', 'Non-Sequential High', 'Non-Sequential Low', 'Sequential High', 'Sequential Low', 'Ground Truth High', 'Ground Truth Low'])
        else:
            # Write back all the existing lines
...
def main():
    ...
    append_to_csv_remove_first(csv_file_name, row)
